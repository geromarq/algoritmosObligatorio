...
namespace FrameworkA1
{
	...
	char *intToStr(int i);
	unsigned int cantDatos(NodoListaIntFramework* l);
	unsigned int cantDatos(NodoListaIntAux* l);
	NodoABInt *convertListaAuxToAB(NodoListaIntAux* & l, int &largo);
	NodoAGIntFramework *convertListaAuxToAG(NodoListaIntAux* & l, int &largo);
	void destruir(NodoListaIntAux*& l);
	void agregarFin(NodoListaIntFramework *&l, int val);
	void abALista(NodoABInt *a, NodoListaIntFramework *&l);
	unsigned int pow2(unsigned pwr);
	unsigned int altura(NodoABInt* a);
	void imprimirListaAux(NodoListaIntFramework *l);
	char *abAStrAux(NodoABInt *a);
	void borrarPpio(NodoListaIntAux* & l);
	void listaAVector(NodoListaIntFramework *l, int *vec, int &pos);
	int *listaAVectorOrdenado(NodoListaIntFramework *l, int &largoVec);
	char *agAStrAux(NodoAGIntFramework *a);
	void abAVector(NodoABInt *a, int *vec, int &pos);
	void agAVector(NodoAGIntFramework *a, int *vec, int &pos);
	int *agAVectorOrdenado(NodoAGIntFramework *a, int &largoVec);
	NodoListaIntAux* splitStringToListaAux(char *stringDatos);
	void convertListaAuxToAG(NodoListaIntAux* &l, NodoAGIntFramework *&a, int &largo);
	...
	void serializarAux(NodoListaIntFramework *l, char *&str);
	NodoAGIntFramework* parsearAG(char *stringDatos, int &largo);
	/*
	PRE:
	...
	*/
	...
	POS:Imprime lista l
	*/
	void imprimirLista(NodoListaIntFramework *l);
	...
	*/
	NodoListaStringFramework* parsearListaStr(char* stringDatos, int& largo);
	/*
	...
	*/
	NodoListaIntFramework *parsearLista(char *stringDatos, int &largo);
	/*
	...
	*/
	NodoListaIntFramework *copiarLista(NodoListaIntFramework *l);
	/*
	PRE:
	...
	*/ 
	...
	unsigned int cantDatos(NodoAGIntFramework* a);
...
void FrameworkA1::imprimirListaAux(NodoListaIntFramework *l) {
	if (l == NULL)
		return;
	cout << l->dato;
	if (l->sig != NULL)
...
void FrameworkA1::imprimirLista(NodoListaIntFramework *l) {
	cout << "{";
	imprimirListaAux(l);
	cout << "}";
}

void FrameworkA1::imprimirEsperadoVsRecibido(NodoListaIntFramework *esp, NodoListaIntFramework *rec) {
...
void FrameworkA1::imprimirEsperadoVsRecibido(NodoListaIntFramework *esp, NodoListaIntFramework *rec, const char *inputParametersFormated) {
...
void FrameworkA1::imprimirEsperadoVsRecibido(NodoAGIntFramework *esp, NodoAGIntFramework *rec, const char *inputParametersFormated) {
	char* esperado = FrameworkA1::serializar(esp);
	char* recibido = FrameworkA1::serializar(rec);
	cout << "\tParametros de entrada: " << inputParametersFormated << endl;
	cout << "\tSe esperaba: " << esperado << endl;
...
char *FrameworkA1::agAStrAux(NodoAGIntFramework *a) {
	char *ret = copioString("");
	if(a==NULL){
		append(ret, ",#");
		return ret;
...
NodoListaIntFramework * FrameworkA1::copiarLista(NodoListaIntFramework *l) {
	if (l == NULL)
		return NULL;
	NodoListaIntFramework *n = new NodoListaIntFramework(l->dato);
...
void FrameworkA1::agregarFin(NodoListaIntFramework *&l, int val)
{
    if (l == NULL)
        l = new NodoListaIntFramework(val);
    else
        agregarFin(l->sig, val);
}

void FrameworkA1::agregarOrd(NodoListaIntFramework*& l, int val)
{
	if (l == NULL || l->dato >= val) {
		NodoListaIntFramework* aux = new NodoListaIntFramework(val);
		aux->sig = l;
		l = aux;
	}
	else
...
void FrameworkA1::listaAVector(NodoListaIntFramework *l, int *vec, int &pos)
{
    if (l != NULL) {
        vec[pos++] = l->dato;
...
int *FrameworkA1::listaAVectorOrdenado(NodoListaIntFramework *l, int &largoVec) {
...
void FrameworkA1::agAVector(NodoAGIntFramework *a, int *vec, int &pos)
{
    if (a != NULL) {
...
int *FrameworkA1::agAVectorOrdenado(NodoAGIntFramework *a, int &largoVec) {
	largoVec = cantDatos(a);
	int *vec = new int[largoVec];
	int pos = 0;
	agAVector(a, vec, pos);
...
unsigned int FrameworkA1::cantDatos(NodoListaIntFramework* l) {
	if (l == NULL)
        return 0;
    return cantDatos(l->sig) + 1;
}
...
unsigned int FrameworkA1::cantDatos(NodoAGIntFramework* a) {
	if (a == NULL)
        return 0;
...
void FrameworkA1::destruir(NodoListaIntFramework *&l) {
    while (l!=NULL) {
        NodoListaIntFramework *aux = l;
        l = l->sig;
        delete aux;
    }
}

void FrameworkA1::destruir(NodoListaStringFramework*& l) {
	while (l != NULL) {
		NodoListaStringFramework* aux = l;
		l = l->sig;
		delete [] aux->dato;
		delete aux;
	}
...
void FrameworkA1::destruir(NodoAGIntFramework *&a) {
    if (a!=NULL) {
...
bool FrameworkA1::sonIgualesDatosForma(NodoListaIntFramework *l1, NodoListaIntFramework *l2) {
    if (l1 == NULL && l2 == NULL)
        return true;
    if (l1 == NULL || l2 == NULL)
        return false;
...
bool FrameworkA1::sonIgualesDatos(NodoListaIntFramework *l1, NodoListaIntFramework *l2) {
	int largo1, largo2;
...
bool FrameworkA1::sonIgualesDatosForma(NodoAGIntFramework *a1,NodoAGIntFramework *a2) {
	if (a1 == NULL && a2 == NULL)
        return true;
    if (a1 == NULL || a2 == NULL)
        return false;
...
bool FrameworkA1::sonIgualesDatos(NodoAGIntFramework *a1, NodoAGIntFramework *a2) {
	int largo1, largo2;
...
bool FrameworkA1::compartenMemoria(NodoListaIntFramework *l1, NodoListaIntFramework *l2) {
	NodoListaIntFramework *l1aux = l1;
	NodoListaIntFramework *l2aux = l2;
	while (l1aux != NULL) {
		l2aux = l2;
		while (l2aux != NULL) {
...
void agAVectorPunteros(NodoAGIntFramework *a1, NodoAGIntFramework **vec, unsigned int &pos) {
	if (a1 != NULL) {
		vec[pos++] = a1;
...
bool FrameworkA1::compartenMemoria(NodoAGIntFramework *a1, NodoAGIntFramework *a2) {
	unsigned int pos1 = 0, pos2 = 0;
	unsigned int cant1 = cantDatos(a1);
	unsigned int cant2 = cantDatos(a2);
	NodoAGIntFramework **vec1 = new NodoAGIntFramework *[cant1];
	NodoAGIntFramework **vec2 = new NodoAGIntFramework *[cant2];
...
int *FrameworkA1::parsearVectorInt(char *stringDatos, int &largo) {
	NodoListaIntFramework *l = parsearLista(stringDatos, largo);
	largo = cantDatos(l);
	if (largo == 0) {
		destruir(l);
		return NULL;
...
NodoListaStringFramework* FrameworkA1::parsearListaStr(char* stringDatos, int& largo) {
	char* str = FrameworkA1::copioString(stringDatos);
	unsigned long len = strlen(str);
	unsigned int startPos = 0;
	bool started = false;
	NodoListaStringFramework* ret = NULL;
	NodoListaStringFramework* ult = NULL;
	...
	for (unsigned int i = 0; i < len; i++) {
		if (str[i] == '\"') {
			if (!started) {
				...
				NodoListaStringFramework* n = new NodoListaStringFramework;
				n->dato = tokenCpy;
				if (ret == NULL) {
					ret = ult = n;
				}
...
NodoListaIntFramework *FrameworkA1::parsearLista(char *stringDatos, int &largo) {
	char *stringDatosC = FrameworkA1::copioString(stringDatos);
	NodoListaIntFramework *ret = NULL;
	NodoListaIntFramework *ant = NULL;
	largo = 0;
	...
	while (token1 != NULL) {
		int val = atoi(token1);
		largo++;
		NodoListaIntFramework *n = new NodoListaIntFramework(val);
		if (ret == NULL) {
			ret = ant = n;
		}
		else {
...
NodoAGIntFramework *FrameworkA1::convertListaAuxToAG(NodoListaIntAux* & l, int &largo){
	if (l == NULL)
		return NULL;

	NodoAGIntFramework* a = NULL;

	
	return a;
}

void FrameworkA1::convertListaAuxToAG(NodoListaIntAux* &l, NodoAGIntFramework *&a, int &largo){
	if (l == NULL)
	{
		a = NULL;
		return;		
	...
	a = new NodoAGIntFramework(l->dato);
	largo++;
	borrarPpio(l);
...
NodoAGIntFramework* FrameworkA1::parsearAG(char *stringDatos, int &largo) {
	NodoListaIntAux *listaAux = splitStringToListaAux(stringDatos);
	largo = 0;
	NodoAGIntFramework* ret;
...
void* FrameworkA1::parsearColeccion(const char *stringDatos, int &largo) {
	...
	NodoABInt *a;
	NodoAGIntFramework *ag;
	NodoListaIntFramework* l;
	NodoListaStringFramework* lStr;
	char **vecStr;
	int *vecInt;
...
void FrameworkA1::serializarAux(NodoListaIntFramework *l, char *&str) {
	if (l == NULL)
		return;
...
char *FrameworkA1::serializar(NodoListaIntFramework *l) {
	char *str;
	if(l!=NULL){
		str = copioString("(");
		serializarAux(l, str);
...
char *FrameworkA1::serializar(NodoAGIntFramework *a) {
	char *str;
	if(a!=NULL){
		str = copioString("{{");
